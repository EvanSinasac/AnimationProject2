...
void main()
{
	...
	if (bBones)
	{
		...
		//if (fVertWorldLocation.y < 7.0f)
		if (fBoneID[0] == 1)
		{
			pixelColour.rgba = vec4(0.0f, 1.0f, 0.0f, 1.0f);
			return;
		}
		//if (fVertWorldLocation.y < 10.0f)
		if (fBoneID[0] == 2)
		{
			pixelColour.rgba = vec4(0.0f, 0.0f, 1.0f, 1.0f);
			return;
		}
		//if (fVertWorldLocation.y < 15.0f)
		if (fBoneID[0] == 3)
		{
			pixelColour.rgba = vec4(1.0f, 1.0f, 0.55f, 1.0f);
			return;
		}
		pixelColour.rgba = vec4(fNormal.xyz, 1.0f);
		return;
	}
