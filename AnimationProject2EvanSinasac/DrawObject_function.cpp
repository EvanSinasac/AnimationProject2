...
void DrawBoneObject(/*cMesh* pCurrentMesh*/cBoneEntity* pCurrentEntity, glm::mat4 matModel,
    ...
    cVAOManager* pVAOManager)
{
    GLint bIsBone_LocID = glGetUniformLocation(program, "bBones");
    glUniform1f(bIsBone_LocID, (GLfloat)GL_TRUE);

    // Set up textures for this object
    SetUpTextures(pCurrentEntity->m_Mesh, program);


    // Is this thing transparent, or an impostor object?
    if ((pCurrentEntity->m_Mesh->alphaTransparency < 1.0f) || (pCurrentEntity->m_Mesh->bIsImposter))
    {
        // Alpha transparency
        glEnable(GL_BLEND);
        // Basic "alpha transparency"
    ...
    // Set the flag if this is an impostor
    GLint bIsImposter_LocID = glGetUniformLocation(program, "bIsImposter");
    if (pCurrentEntity->m_Mesh->bIsImposter)
    {
        glUniform1f(bIsImposter_LocID, (float)GL_TRUE);
    }
    else
    ...
    GLint wholeObjectAlphaTransparency_LocID = glGetUniformLocation(program, "wholeObjectAlphaTransparency");
    //Set this value here
    glUniform1f(wholeObjectAlphaTransparency_LocID, pCurrentEntity->m_Mesh->alphaTransparency);

    // *****************************************************
    // Translate or "move" the object somewhere
    glm::mat4 matTranslate = glm::translate(glm::mat4(1.0f),
        pCurrentEntity->m_Mesh->positionXYZ);
    //matModel = matModel * matTranslate;
    // *****************************************************
    ...
    // Rotation around the Z axis
    glm::mat4 rotateZ = glm::rotate(glm::mat4(1.0f),
        pCurrentEntity->m_Mesh->orientationXYZ.z,//(float)glfwGetTime(),
        glm::vec3(0.0f, 0.0f, 1.0f));
    //matModel = matModel * rotateZ;
    // *****************************************************
    ...
    // Rotation around the Y axis
    glm::mat4 rotateY = glm::rotate(glm::mat4(1.0f),
        pCurrentEntity->m_Mesh->orientationXYZ.y,
        glm::vec3(0.0f, 1.0f, 0.0f));
    //matModel = matModel * rotateY;
    // *****************************************************
    ...
    // Rotation around the X axis
    glm::mat4 rotateX = glm::rotate(glm::mat4(1.0f),
        pCurrentEntity->m_Mesh->orientationXYZ.x,
        glm::vec3(1.0f, 0.0f, 0.0f));
    //matModel = matModel * rotateX;
    // *****************************************************
    ...
    // just trying this real quick
    //matModel = pCurrentEntity->p_mBoneHierarchy->rootBone->GetModelMatrix();
    ...
    if (pCurrentEntity->m_Mesh->bUseWholeObjectDiffuseColour)
    {
        glUniform1f(bUseWholeObjectDiffuseColour_Location, (float)GL_TRUE);
        glUniform4f(wholeObjectDiffuseColour_Location,
            pCurrentEntity->m_Mesh->wholeObjectDiffuseRGBA.r,
            pCurrentEntity->m_Mesh->wholeObjectDiffuseRGBA.g,
            pCurrentEntity->m_Mesh->wholeObjectDiffuseRGBA.b,
            pCurrentEntity->m_Mesh->wholeObjectDiffuseRGBA.a);
    }
    else
    {
        glUniform1f(bUseWholeObjectDiffuseColour_Location, (float)GL_FALSE);
    ...
    glUniform4f(wholeObjectSpecularColour_Location,
        pCurrentEntity->m_Mesh->wholeObjectSpecularRGB.r,
        pCurrentEntity->m_Mesh->wholeObjectSpecularRGB.g,
        pCurrentEntity->m_Mesh->wholeObjectSpecularRGB.b,
        pCurrentEntity->m_Mesh->wholeObjectShininess_SpecPower);


    // See if mesh is wanting the vertex colour override (HACK) to be used?
    if (pCurrentEntity->m_Mesh->bUseObjectDebugColour)
    {
        // Override the colour...
        glUniform1f(bUseDebugColour_Location, (float)GL_TRUE);
        glUniform4f(objectDebugColour_Location,
            pCurrentEntity->m_Mesh->objectDebugColourRGBA.r,
            pCurrentEntity->m_Mesh->objectDebugColourRGBA.g,
            pCurrentEntity->m_Mesh->objectDebugColourRGBA.b,
            pCurrentEntity->m_Mesh->objectDebugColourRGBA.a);
    }
    else
    {
        // DON'T override the colour
    ...
    // See if mesh is wanting the vertex colour override (HACK) to be used?
    if (pCurrentEntity->m_Mesh->bDontLight)
    {
        // Override the colour...
        glUniform1f(bDontLightObject_Location, (float)GL_TRUE);
    }
    ...
    // Wireframe
    if (pCurrentEntity->m_Mesh->bIsWireframe)                // GL_POINT, GL_LINE, and GL_FILL)
    {
        // Draw everything with only lines
        glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    }
    ...
    // Depth buffer
    if (pCurrentEntity->m_Mesh->bDisableDepthBufferCheck)
    {
        glDisable(GL_DEPTH_TEST);
    }
    else
    ...
    // Send Bone values
    BoneHierarchy* boneHierarchy = pCurrentEntity->p_mBoneHierarchy;

    //int numBones = 48;
    int numBones = boneHierarchy->bones.size();
    for (int i = 0; i < numBones; i++)
    {
        glm::mat4 BoneTranslationMatrix = glm::translate(glm::mat4(1.0f), boneHierarchy->bones[i]->GetPosition());
        glm::mat4 BoneRotationMatrix = glm::mat4_cast(boneHierarchy->bones[i]->GetRotation());
        glm::mat4 BoneScaleMatrix = glm::scale(glm::mat4(1.0f), boneHierarchy->bones[i]->GetScale());
        glm::mat4 BoneModelMatrix = BoneTranslationMatrix * BoneRotationMatrix * BoneScaleMatrix;

        std::stringstream ss;
        ss << "BoneMatrices[" << i << "]";
        GLint mBoneMatrixULs = glGetUniformLocation(program, ss.str().c_str());

        ss.str("");
        ss << "BoneRotationMatrices[" << i << "]";
        GLint mBoneRotationMatrixULs = glGetUniformLocation(program, ss.str().c_str());

        glUniformMatrix4fv(mBoneMatrixULs, 1, GL_FALSE, glm::value_ptr(boneHierarchy->bones[i]->GetModelMatrix()));
        //glUniformMatrix4fv(mBoneMatrixULs, 1, GL_FALSE, glm::value_ptr(glm::mat4(1.0f)));
        //glUniformMatrix4fv(mBoneMatrixULs, 1, GL_FALSE, glm::value_ptr(BoneModelMatrix));
        glUniformMatrix4fv(mBoneRotationMatrixULs, 1, GL_FALSE, glm::value_ptr(boneHierarchy->bones[i]->GetRotation()));
        //glUniformMatrix4fv(mBoneRotationMatrixULs, 1, GL_FALSE, glm::value_ptr(BoneRotationMatrix));

        int breakhere = 0;
    }

    // I should attach these weights and ids to the bone entity later
    GLint mBoneIDs_LocID = glGetUniformLocation(program, "in_BoneIDs");
    glUniform4f(mBoneIDs_LocID, 0.0f, 1.0f, 2.0f, 3.0f);    

    GLint mBoneWeights_LocID = glGetUniformLocation(program, "in_BoneWeights_x4");
    glUniform4f(mBoneWeights_LocID, 1.0f, 1.0f, 1.0f, 1.0f);
    //glUniform4f(mBoneWeights_LocID, 1.0f, 2.0f, 3.0f, 4.0f);
    ...
    if (pVAOManager->FindDrawInfoByModelName(pCurrentEntity->m_Mesh->meshName, modelInfo))
    {
        glBindVertexArray(modelInfo.VAO_ID);
    ...
    //for (std::vector<cMesh*>::iterator itChildMesh = pCurrentMesh->vec_pChildMeshes.begin();
    //    itChildMesh != pCurrentMesh->vec_pChildMeshes.end(); itChildMesh++)
    for (std::vector<cBoneEntity*>::iterator itBoneChild = pCurrentEntity->GetChildren().begin();
        itBoneChild != pCurrentEntity->GetChildren().end(); itBoneChild++)
    {
        //cMesh* pChildMesh = *itChildMesh;
        cBoneEntity* pChildBone = *itBoneChild;
    ...
    glUniform1f(bIsBone_LocID, (GLfloat)GL_FALSE);
